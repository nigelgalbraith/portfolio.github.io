...
// Embedded main text data for GitHub Pages (no fetch required)
...
// Injects main content into any element with a data-main-text attribute
document.addEventListener("DOMContentLoaded", () => {
  const target = document.querySelector('[data-main-text]');
  if (!target) return;

  const key = target.dataset.mainText;
  const content = MAIN_TEXT_DATA[key];
  if (!content) return;
  ...
  content.lists?.forEach(list => {
    ...
    list.items.forEach(item => {
      ...
      li.innerHTML = item; // Allow HTML content like <a> links
      ul.appendChild(li);
    });
