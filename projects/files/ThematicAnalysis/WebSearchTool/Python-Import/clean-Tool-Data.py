...
# Define constants
INPUT_JSON_FILE = "json_files/tool.json"
OUTPUT_JSON_FILE = "json_files/tool-clean.json"
OUTPUT_JS_FILE = "json_files/toolJSON.js"
KEYS_TO_SPLIT_ONE = ["Concern", "Tech Concepts", "Organizational Value"]
KEYS_TO_SPLIT_TWO = ["Tech Layers"]
KEYS_TO_SPLIT_THREE = ["Possible Solution"]
JSON_PREFIX = "jsonData"
KEY_BROAD_TECH = "Organizational Value"
KEY_TECH_CON = "Tech Concepts"
KEY_TECH_LAYER = "Tech Layers"
KEY_POSSIBLE_SOLUTIONS = "Possible Solution"
KEY_CHALLENGE = "Concern"
CONCEPTS = "Concepts"
NEWLINE = "\n"
DOUBLE_NEWLINE = "\n\n"
...
if __name__ == "__main__":
    ...
    if data:
        ...
        # Remove leading and trailing spaces from all keys
        data = trim_spaces_in_keys(data)
        
        # Remove uni characters
        data = remove_text_from_json(data, SPECIFIC_TEXT_REMOVE)
        
        # Split specified values into lists
        split_values(data, KEYS_TO_SPLIT_ONE, NEWLINE)
        split_values(data, KEYS_TO_SPLIT_TWO, NEWLINE)
        split_values(data, KEYS_TO_SPLIT_THREE, NEWLINE)

        # Associate the first 'Broad Tech Concept' with the first 'Technology Concept'
        associate_keys_list(data, KEY_BROAD_TECH, KEY_TECH_CON, CONCEPTS)

        # Associate the first 'Technology Layers' with the first 'Technology Concept'
        add_values_to_key(data, CONCEPTS, KEY_POSSIBLE_SOLUTIONS)

        # Associate the first 'Technology Layers' with the first 'Technology Concept'
        add_values_to_key(data, CONCEPTS, KEY_TECH_LAYER)

        # Associate the first 'Technology Layers' with the first 'Technology Concept'
        add_values_to_key(data, CONCEPTS, KEY_CHALLENGE)

        # Standardize keys items to title
        data = title_key_list_values(data, KEY_TECH_CHALL, ITEM_IGNORE_VALUE)
        
        # Write modified JSON data to file
        write_json(data, OUTPUT_JSON_FILE)

        # Add prefix and save as a JavaScript file
